  ...
  const unitClick = async (modulo, unit) => {
    if (unit.is_lock) {
      ...
      if (type === "sidebar") {
        ...
        history.push({
          pathname: `${current_course_units.slugCourse}/${modulo.slug}/${unit.slug}`,
        });
      }
    }
  };
    ...
    <div>
      {aulas.length ? (
        <div className="header-side">
          <CircularProgressbar
            ...
            className="progress"
            value={
              (parseFloat(finished_aux.split("/")[0]) /
                parseFloat(finished_aux.split("/")[1])) *
              100
            }
            text={`${parseInt(
              (finished_aux.split("/")[0] / finished_aux.split("/")[1]) * 100
            )}%`}
          />
      ...
      {aulas.map((item, index) => (
        <ExpansionPanel key={`expansion-panel-${index}`}>
          ...
          >
            {item.finished ? (
              <CheckCircle style={{ color: "#28A745", paddingRight: 20 }} />
            ) : (
              <CheckCircle style={{ color: "#DDD", paddingRight: 20 }} />
            )}
